...
const EXPORTED_AEM_ASSETS_ROOT = 'cmscontent';
...
class AzureIngestor extends FileIngestor {
    ...
    async init() {
        ...
        this.customAzureFilesSdk = await Files.init(
            {
                azure: {
                    sasURLPublic: this.params.sasURLAzurePublic,
                    sasURLPrivate: this.params.sasURLAzurePrivate,
                }
            });
    }
    ...
    async ingestAemAsset(aemAssetPath) {
        ...
        //     });
            ...
            await this.customAzureFilesSdk.write(EXPORTED_AEM_ASSETS_ROOT, "Text test");

            // logger.info(`asset "${aemAssetUrl}", size: ${bytes}, is downloaded at ${new Date().toISOString()}`);
            return { 'bytesWrittenToAzure': "Text test" };
        // }
    }
}
